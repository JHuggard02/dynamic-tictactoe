...
export default function App() {
  ...
  const makeMove = (row, col) => {
    ...
    try {
      socket.send(JSON.stringify({ "update-game": true, start: false, move: [row, col] }));
    } catch (e) {
  ...
  const startGame = () => {
    ...
    try{
      socket.send(JSON.stringify({"update-game": true, start: true}));
    }catch (e){
  ...
  const createGame = () => {
    ...
    try {
      socket.send(JSON.stringify({"update-player": true, username, gameID, create: true, join: false, rows, cols, "win-cond": winCond}))
    }catch (e) {
  ...
  const joinGame = () => {
    ...
    try {
      socket.send(JSON.stringify({"update-player": true, username, gameID, create: false, join: true}))
    }catch (e) {
      console.log("Error Sending User Info", e);
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-400 to-blue-600 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-2xl p-8  win-w-[28rem]">
        ...
        {
            joined && <Board getWinnerMessage={getWinnerMessage} board={board} makeMove={makeMove} getCellClass={getCellClass} winner={winner} currentPlayer={currentPlayer} username={username} resetGame={resetGame} connected={connected} reset={reset}/>
        }

        {
            waiting && <Waiting startGame={startGame} players={players}/>
        }

        {
            (!joined && !waiting) && <JoinGame updateGameID={updateGameID} updateUsername={updateUsername} username={username} gameID={gameID} createGame={createGame} joinGame={joinGame} rows={rows} setRows={setRows} cols={cols} setCols={setCols} winCond={winCond} setWinCond={setWinCond}/>
        }
        ...
        {error && <h1 className="mt-5 text-center text-red-600 text-sm">{error}</h1>}
